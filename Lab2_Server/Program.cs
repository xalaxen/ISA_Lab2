...
namespace Lab2_Server
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    await Task.Run(() => ProcessRequest(ref result, ref p, ref f, ref semaphore));
                }
                catch (Exception ex)
                {
        ...
        private static void ProcessRequest(ref UdpReceiveResult result, ref Program p, ref Functions f, ref SemaphoreSlim semaphore)
        {
            semaphore.Wait();
            ...
            try
            {
                ...
                using (StudentContext db = new StudentContext())
                {
                    ...
                    {
                        case "1":
                            response = f.PrintAllNotes(ref s);
                        ...
                        case "2":
                            ...
                            try
                            {
                                ...
                                response = f.PrintNotesByNumber(number_to_print, ref s);
                                logger.Info($"Server sent: {response}");
                                p.SendMessage(response + "\n" + p.Menu());
                            }
                            catch (Exception ex)
                        ...
                        case "4":
                            ...
                            try
                            {
                                ...
                                f.RemoveNotesFromFile(number_to_delete);
                                logger.Debug($"Item with id {number_to_delete} deleted");
